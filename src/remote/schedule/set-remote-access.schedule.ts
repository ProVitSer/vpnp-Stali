...
export class SetRemoteAccessScheduleService {
    ...
    async activateRemoteAccess() {
        try {
            const result = await this.remoteModelService.findByCriteria({ status: RemoteStatus.inProgress, dateFrom: format(new Date(), 'dd.MM.yyyy') });
            if (result.length === 0) return;
            await Promise.all(result.map(async user => {
                await this.remoteService.activateRemoteAccess({ ...this.getRemoteAccessData(user, RemoteAccessStatus.activate) as RemoteActivateDtoWithId })
            }))
        } catch (e) {
    ...
    async deactivateRemoteAccess() {
        try {
            const result = await this.remoteModelService.findByCriteria({ status: RemoteStatus.completed, dateTo: format(new Date(), 'dd.MM.yyyy') });
            if (result.length === 0) return;
            await Promise.all(result.map(async user => {
